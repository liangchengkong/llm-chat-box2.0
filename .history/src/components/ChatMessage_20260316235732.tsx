...
function ChatMessage({ message, isLastAssistantMessage, onRegenerate }: ChatMessageProps) {
  ...
  const handleContentClick = useCallback(async (event: React.MouseEvent) => {
    const target = event.target as HTMLElement
    const actionBtn = target.closest('[data-action]') as HTMLElement
    if (!actionBtn) return

    const action = actionBtn.dataset.action
    const codeBlock = actionBtn.closest('.code-block')
    ...
    if (action === 'copy' && codeBlock) {
      ...
      if (code) {
        try {
          ...
          const blockId = codeBlock.querySelector('code')?.textContent?.slice(0, 20) || Date.now().toString()
    ...
    if (action === 'theme' && codeBlock) {
      const themeIcon = actionBtn.querySelector('img') as HTMLImageElement
